...
const searchElement = mainElement.querySelector(`.main__search`);
...
render(menuElement, getMenuMarkup());
render(searchElement, getSearchMarkup());
render(filterElement, getFilterMarkup());
render(boardElement, getSortMarkup(), `afterbegin`);
render(tasksElement, getCardFormMarkup());

for (let i = 0, n = 3; i < n; i++) {
  render(tasksElement, getCardMarkup());
}
